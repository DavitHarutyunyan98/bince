...
app.layout = html.Div(style={'backgroundColor': '#111111', 'color': '#FFFFFF', 'padding': '10px'}, children=[
    ...
    html.Div([
        ...
        html.A('Backtest Results', href='#manual-results-section',
               className='nav-link'),
        html.A('ADX Scanner', href='#manual-results-section',
               className='nav-link'),
    ...
    html.Div(id='manual-results-section', children=[
        # MOVED TO TOP: Market Trend Scanner (ADX) Module - Primary Focus
        create_collapsible_container("Market Trend Scanner (ADX)", "adx-scanner", [
            # Row 1: Controls with dedicated ADX Date Picker
            html.Div([
                html.H4("ADX Scanner Configuration", style={'textAlign': 'center', 'marginBottom': '20px'}),
                
                # First Row: Timeframe, Lookback, Scan Limit, Date Range
                html.Div([
                    html.Div([
                        html.Label("Timeframe:", style={'fontWeight': 'bold'}),
                        dcc.Dropdown(
                            id='adx-timeframe-dropdown',
                            options=[
                    ...
                    html.Div([
                        html.Label("Lookback (Candles):", style={'fontWeight': 'bold'}),
                        dcc.Input(
                            id='adx-lookback-input',
                            type='number',
                            value=200,
                            min=50,
                            max=1000,
                            step=50,
                            style={'width': '100%', 'padding': '5px', 'backgroundColor': '#2c2c2c', 
                                   'color': '#f0f0f0', 'border': '1px solid #555'}
                        )
                    ], className='flex-item'),
                    
                    html.Div([
                        html.Label("Scan Limit (Top N Pairs):", style={'fontWeight': 'bold'}),
                        dcc.Input(
                            id='scan-limit-input',
                            type='number',
                            value=20,
                    ...
                    html.Div([
                        html.Label("ADX Chart Date Range:", style={'fontWeight': 'bold'}),
                        dcc.DatePickerRange(
                            id='adx-date-picker',
                ...
                ], className='flex-container', style={'marginBottom': '15px'}),
                
                # Second Row: ADX Configuration + Action
                html.Div([
                    html.Div([
                        html.Label("DI Length:", style={'fontWeight': 'bold'}),
                        dcc.Input(
                            id='di-length-input',
                            type='number',
                            value=14,
                    ...
                    html.Div([
                        html.Label("ADX Smoothing:", style={'fontWeight': 'bold'}),
                        dcc.Input(
                            id='adx-smoothing-input',
                            type='number',
                            value=14,
                            min=5,
                            max=50,
                    ...
                    html.Div([
                        html.Label("Strong Trend Threshold:", style={'fontWeight': 'bold'}),
                        dcc.Input(
                            id='strong-trend-threshold-input',
                            type='number',
                            value=25,
                            min=10,
                            max=50,
                    ...
                    html.Div([
                        html.Label("Weak Trend Threshold:", style={'fontWeight': 'bold'}),
                        dcc.Input(
                            id='weak-trend-threshold-input',
                            type='number',
                            value=20,
                            min=5,
                            max=40,
                    ...
                    html.Div([
                        html.Label("Manual Search:", style={'fontWeight': 'bold'}),
                        dcc.Input(
                            id='adx-search-input',
                            type='text',
                            placeholder="Symbol (e.g. BTCUSDT)",
                            style={'width': '100%', 'padding': '5px', 'backgroundColor': '#2c2c2c', 
                                   'color': '#f0f0f0', 'border': '1px solid #555', 'marginBottom': '5px'}
                        ),
                        html.Button(
                            "Analyze Pair",
                            id='adx-search-btn',
                    ...
                    html.Div([
                        html.Label("Market Scan:", style={'fontWeight': 'bold'}),
                        html.Button(
                            "Scan Market",
                            id='scan-market-btn',
                            n_clicks=0,
                ...
                ], className='flex-container', style={'marginBottom': '20px'}),
            ]),
            
            # Row 2: Results Table (Full Width)
            html.Div([
                html.H4("ADX Scan Results", style={'textAlign': 'center', 'marginBottom': '10px'}),
                html.Div([
                    html.Label("Select Symbol for Chart:", style={'fontWeight': 'bold', 'marginBottom': '5px'}),
                    dcc.Dropdown(
                        id='adx-symbol-dropdown',
                        placeholder="Select a symbol to view ADX chart...",
                        style={'backgroundColor': '#2c2c2c', 'color': '#f0f0f0', 'width': '300px'}
                    )
                ], style={'marginBottom': '15px', 'textAlign': 'center'}),
                dash_table.DataTable(
                    id='adx-results-table',
                    columns=[
                        {'name': 'Symbol', 'id': 'Symbol'},
                        {'name': 'Current Price', 'id': 'Current Price'},
                        {'name': 'ADX Value', 'id': 'ADX Value'},
                        {'name': 'Trend Strength', 'id': 'Trend Strength'}
                    ],
                    data=[],
                    sort_action='native',
                    ...
                    style_data_conditional=[],  # Will be populated dynamically
                    page_size=10
                )
            ], style={'marginBottom': '30px'}),
            
            # Row 3: ADX Chart (Full Width - Primary Focus)
            html.Div([
                html.H4("ADX Trend Analysis Chart", style={'textAlign': 'center', 'marginBottom': '15px'}),
                dcc.Graph(
                    id='adx-chart',
                    style={'height': '70vh', 'width': '100%'},  # Full width, taller for primary focus
                    figure=go.Figure().update_layout(
                        title="Select a symbol to view ADX analysis",
...
# --- ADX Scanner Callbacks ---
@app.callback(
    [Output('adx-results-table', 'data'),
     Output('adx-results-table', 'style_data_conditional'),
     Output('adx-symbol-dropdown', 'options'),
     Output('adx-symbol-dropdown', 'value')],
    [Input('scan-market-btn', 'n_clicks'),
     Input('adx-search-btn', 'n_clicks')],
    [State('adx-timeframe-dropdown', 'value'),
     State('adx-lookback-input', 'value'),
     State('scan-limit-input', 'value'),
     State('di-length-input', 'value'),
     State('adx-smoothing-input', 'value'),
     State('strong-trend-threshold-input', 'value'),
     State('weak-trend-threshold-input', 'value'),
     State('adx-search-input', 'value')],
    prevent_initial_call=True
)
def scan_market_for_adx(scan_clicks, search_clicks, timeframe, lookback_candles, scan_limit, di_length, 
                       adx_smoothing, strong_threshold, weak_threshold, search_symbol):
    """Callback 1: Market scanning and manual pair analysis for ADX."""
    if not trader or (not scan_clicks and not search_clicks):
        return [], [], [], None
    ...
    try:
        if button_id == 'scan-market-btn':
            # MARKET SCAN: Run the existing logic (fetch top N pairs)
            results = trader.scan_market_adx(
                scan_limit=scan_limit or 20,
                timeframe=timeframe or '1h',
                lookback_candles=lookback_candles or 200,
                di_length=di_length or 14,
                adx_smoothing=adx_smoothing or 14,
                strong_threshold=strong_threshold or 25,
                weak_threshold=weak_threshold or 20
            )
            
            if not results:
                return [], [], [], None
            
            # Create dropdown options for symbol selection
            dropdown_options = [{'label': result['Symbol'], 'value': result['Symbol']} 
                               for result in results]
            
            # Auto-select the first symbol (highest ADX) for immediate chart rendering
            default_symbol = results[0]['Symbol'] if results else None
            
        elif button_id == 'adx-search-btn':
            # MANUAL SEARCH: Analyze single pair
            if not search_symbol or search_symbol.strip() == '':
                print("DEBUG: Empty search symbol")
                return [], [], [], None
            
            # Validate and format symbol
            symbol = search_symbol.strip().upper()
            if not symbol.endswith('USDT'):
                symbol += 'USDT'  # Auto-append USDT if not present
            
            print(f"DEBUG: Analyzing single pair: {symbol}")
            ...
            minutes_needed = (lookback_candles or 200) * timeframe_minutes.get(timeframe or '1h', 60)
            start_date = (datetime.now() - timedelta(minutes=minutes_needed * 2)).strftime('%Y-%m-%d')
            end_date = datetime.now().strftime('%Y-%m-%d')
            
            # Fetch historical data for the single symbol
            data = trader.get_historical_data_for_symbol(symbol, timeframe or '1h', start_date, end_date)
            if data is None or data.empty:
                print(f"DEBUG: No data available for {symbol}")
                return [], [], [], None
            
            # Ensure we have enough data
            if len(data) < max(di_length or 14, adx_smoothing or 14) + 10:
                print(f"DEBUG: Insufficient data for {symbol}")
                return [], [], [], None
            
            # Take only the last lookback_candles for analysis
            data = data.tail(lookback_candles or 200)
            
            # Calculate ADX with separate DI length and ADX smoothing
            data_with_adx = trader.calculate_adx(data, di_length=di_length or 14, adx_smoothing=adx_smoothing or 14)
            
            if data_with_adx.empty or 'ADX' not in data_with_adx.columns:
                print(f"DEBUG: ADX calculation failed for {symbol}")
                return [], [], [], None
            ...
            # Skip if ADX is still 0 (not enough data for calculation)
            if latest_adx == 0:
                print(f"DEBUG: ADX is 0 for {symbol} - insufficient data")
                return [], [], [], None
            
            # Determine trend strength using configurable thresholds
            if latest_adx >= (strong_threshold or 25):
                trend_strength = "Strong"
            elif latest_adx >= (weak_threshold or 20):
                trend_strength = "Moderate"
            else:
                trend_strength = "Weak"
            ...
            # Create dropdown options for symbol selection
            dropdown_options = [{'label': symbol, 'value': symbol}]
            
            # Auto-select the analyzed symbol
            default_symbol = symbol
            
        else:
            return [], [], [], None
        
        # Create dynamic styling based on configurable thresholds
        style_conditions = [
            {
                'if': {'filter_query': f'{{ADX_Numeric}} >= {strong_threshold or 25}'},
                'backgroundColor': '#1e5631',
                'color': '#4caf50'
            },
            {
                'if': {'filter_query': f'{{ADX_Numeric}} >= {weak_threshold or 20} && {{ADX_Numeric}} < {strong_threshold or 25}'},
                'backgroundColor': '#4a4a1e',
                'color': '#ffeb3b'
            },
            {
                'if': {'filter_query': f'{{ADX_Numeric}} < {weak_threshold or 20}'},
                'backgroundColor': '#4a4a4a',
                'color': '#9e9e9e'
            }
        ]
        
        # Remove ADX_Numeric from display data (keep for styling)
        display_results = [{k: v for k, v in result.items() if k != 'ADX_Numeric'} 
                          for result in results]
        
        return display_results, style_conditions, dropdown_options, default_symbol
        
    except Exception as e:
        logger.error(f"Error in ADX scan callback: {e}")
        return [], [], [], None


@app.callback(
    Output('adx-chart', 'figure'),
    [Input('adx-symbol-dropdown', 'value'),
     Input('adx-results-table', 'selected_rows'),
     Input('adx-date-picker', 'start_date'),
     Input('adx-date-picker', 'end_date')],
    [State('adx-timeframe-dropdown', 'value'),
     State('di-length-input', 'value'),
     State('adx-smoothing-input', 'value'),
     State('strong-trend-threshold-input', 'value'),
     State('adx-results-table', 'data')],
    prevent_initial_call=True
)
def update_adx_chart(selected_symbol, selected_rows, start_date, end_date, timeframe, di_length, adx_smoothing, 
                    strong_threshold, table_data):
    """Callback 2: Update ADX chart based on symbol selection with configurable parameters."""
    
    # Debug logging
    print(f"DEBUG ADX CHART: Symbol={selected_symbol}, Dates={start_date}-{end_date}")
    ...
    # Determine symbol from dropdown or table selection
    symbol = None
    if selected_symbol:
    ...
    if not symbol or symbol == "":
        return go.Figure().update_layout(
            title="Select a pair from the dropdown or Scan Market",
            template='plotly_dark',
            paper_bgcolor='#111111',
            plot_bgcolor='#1e1e1e',
            annotations=[
                dict(
                    text="Select a pair from the dropdown or Scan Market",
    ...
    try:
        # Handle None dates gracefully - default to 60 days ago and today
        if not start_date or not end_date:
            chart_end_date = datetime.now().strftime('%Y-%m-%d')
            chart_start_date = (datetime.now() - timedelta(days=60)).strftime('%Y-%m-%d')
        else:
            chart_start_date = start_date
            chart_end_date = end_date
        
        # Fetch historical data for the selected symbol with specified timeframe
        data = trader.get_historical_data_for_symbol(symbol, timeframe or '1h', 
        ...
        # Calculate ADX with separate DI length and ADX smoothing parameters
        data_with_adx = trader.calculate_adx(data, di_length=di_length or 14, 
                                           adx_smoothing=adx_smoothing or 14)
        
        if 'ADX' not in data_with_adx.columns:
            return go.Figure().update_layout(
                title=f"ADX calculation failed for {symbol}",
                template='plotly_dark',
                paper_bgcolor='#111111',
                plot_bgcolor='#1e1e1e'
            )
        ...
        # Add ADX line
        fig.add_trace(go.Scatter(
            x=data_with_adx.index,
            y=data_with_adx['ADX'],
            mode='lines',
            name='ADX',
            line=dict(color='#00BFFF', width=3),
            hovertemplate='<b>ADX</b>: %{y:.2f}<br><b>Date</b>: %{x}<extra></extra>'
        ))
        
        # Add +DI line
        if '+DI' in data_with_adx.columns:
            fig.add_trace(go.Scatter(
                x=data_with_adx.index,
                y=data_with_adx['+DI'],
                mode='lines',
                ...
                hovertemplate='<b>+DI</b>: %{y:.2f}<br><b>Date</b>: %{x}<extra></extra>'
            ))
        ...
        strong_threshold_value = strong_threshold or 25
        fig.add_hline(
            y=strong_threshold_value,
            line_dash="dash",
            line_color="orange",
            annotation_text=f"Strong Trend ({strong_threshold_value})",
            annotation_position="bottom right"
        )
        
        # Add weak threshold line
        fig.add_hline(
            y=20,
            line_dash="dot",
            line_color="gray",
        ...
        fig.update_layout(
            title=f"{symbol} ({timeframe}) - ADX: {latest_adx:.2f} | {trend_direction} | DI Length: {di_length or 14} | ADX Smoothing: {adx_smoothing or 14}",
            xaxis_title="Date",
            yaxis_title="ADX / DI Values",
            template='plotly_dark',
            paper_bgcolor='#111111',
            plot_bgcolor='#1e1e1e',
            hovermode='x unified',
            ...
            yaxis=dict(range=[0, max(100, latest_adx * 1.2)])
        )
        
        return fig
        
    except Exception as e:
        logger.error(f"Error creating ADX chart for {symbol}: {e}")
        return go.Figure().update_layout(
